...
export class DiscordInteractionModule extends Module {
	...
	public async load() {
		...
		try {
			this.client.application?.commands.set(
				this.crossbuild!.components.filter((x) => x.type === "command").map(
					(command) => {
	...
	public async startListening() {
		...
		if (
			!this.crossbuild?.modules.find((x) => x instanceof DiscordMessageModule)
		) {
	...
	private async interaction(discordInteraction: Interaction<CacheType>) {
		...
		const interaction = new DiscordReceivedInteraction(this.crossbuild, {
			...
			key:
				discordInteraction.isCommand() || discordInteraction.isAutocomplete()
