...
export class DiscordInteractionModulePaginator extends ModulePaginator {
    public createPaginatorMessage(
        ...
        const done = { ...message }
        if (done.components) {
            done.components.push(generatedButtons[0])
        } else {
            done.components = generatedButtons
        }
        return done
    }
    ...
    public async handlePage(
        ...
        const message = this.createPaginatorMessage(
            paginator.pages[page - 1],
            paginator.getPrevNext(page),
            paginator.id
        )
    ...
    private generateDiscordComponents(
        prevNext: PrevNext,
        id: string
    ): NonNullable<GeneratedMessageObject["components"]> {
        const row = {
            type: 1,
            components: [
                {
