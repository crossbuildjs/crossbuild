		...
		switch (shouldBe.type) {
			case "string": {
				if (typeof option !== "string")
					return this.errorData.push(
						`Expected type string for ${shouldBe.name}, got ${typeof option}!}`
					)
				if (shouldBe.choices) {
			...
			case "integer": {
				...
				if (typeof num !== "number")
					return this.errorData.push(
						`Expected type integer for ${shouldBe.name}, got ${typeof option}!`
					)
			...
			case "number": {
				...
				if (typeof num !== "number")
					return this.errorData.push(
						`Expected type number for ${shouldBe.name}, got ${typeof option}!`
					)
				if (shouldBe.minValue) {
					if (num < shouldBe.minValue) {
						return this.errorData.push(
			...
			case "boolean": {
				if (typeof option !== "boolean")
					return this.errorData.push(
						`Expected type boolean for ${shouldBe.name}, got ${typeof option}!`
					)
				return option
			}
			case "user":
