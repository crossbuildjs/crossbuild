import { CrossBuild, GeneratedMessageObject, ReceivedInteraction } from ".."
...
export type PaginatorMessage = Pick<
    GeneratedMessageObject,
    "embeds" | "content" | "components"
>
...
export class Paginator {
    ...
    public async send(interaction: ReceivedInteraction, page = 1) {
        const channel = interaction.channel
        if (!channel)
            return console.error("No channel provided to send paginator to.")
    ...
    private async moduleSend(
        data: GeneratedMessageObject,
        interaction: ReceivedInteraction
    ) {
        const module = this.getModule(interaction.source)
        await module.modulePaginator.sendPaginatorMessage(
            data,
            interaction,
            this
        )
    ...
    private createMessage(
        ...
        const message = module.modulePaginator.createPaginatorMessage(
            data,
            this.getPrevNext(page),
            this.id
        )
        return message
    }
