...
import {
    ...
    GuildedEmojiID,
    GuildedMessage,
    GuildedModulePaginator,
    GuildedOptionsHandler,
    GuildedServer,
    GuildedUser
} from "."
...
export class GuildedModule extends Module {
    ...
    public async startListening() {
        ...
        this.client.on("messageReactionCreated", async (reaction) => {
            ...
            if (!message)
                throw new Error("Unable to find Guilded message for paginator.")
            const paginator = this.modulePaginator.getMessagesPaginator(
                message.id
            )
            if (!paginator) return
            await message.deleteReaction(reaction.emote.id, reaction.createdBy)
            if (reaction.createdBy !== paginator.settings.userId) return
            const { previous, next } = paginator.getPrevNext(
    ...
    public optionsHandler(
        ...
        return new GuildedOptionsHandler(interaction, componentOptions)
    }
    ...
    private async message(guildedMessage: Message) {
        ...
        const args = guildedMessage.content
        ...
        let match
        // biome-ignore lint/suspicious/noAssignInExpressions: this is the cleanest way to do this
        ...
        const channel: GuildedChannel | null = guildedMessage.channel
            ...
            ? new GuildedChannel(
                  await this.client.channels.fetch(guildedMessage.channelId)
              )
            : null
        ...
        const server: GuildedServer | null = guildedMessage.server
            ...
            ? new GuildedServer(
                  await this.client.servers.fetch(guildedMessage.serverId)
              )
            : null

        const user: GuildedUser | null = guildedMessage.author
            ? new GuildedUser(guildedMessage.author)
            : null
